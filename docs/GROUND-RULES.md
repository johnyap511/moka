# MOKA ground rules (DRAFT — pending confirmation, 4 Sep 2026)

These rules apply everywhere: sync, auto-assign, hourly reconcile, calendar, owner portal, both exports, Cowork reports.
Nothing in the platform may follow a different rule. Confirmed items become "RULE"; open items are marked "CONFIRM".
...
2. Every eZee stay exists in MOKA with its RES number. A row without a RES is allowed only for a MOKA-only booking
   that does not exist in eZee. CONFIRM: link the 50 hand-keyed August pieces of eZee stays to their RES.
...
7. Cleaning fee = eZee's "Cleaning Fee" line. If eZee has no cleaning line but a "Channel" line, that line is the cleaning fee.
   Deposits, late check-out and other incidentals are never cleaning. If eZee bills room only, cleaning = 0.
...
9. OTA fee:
   ...
   - Direct channels (Walk In, PMS, Google, Internet, Booking Engine): shown as "Website", 8% M&A fee.
     CONFIRM: fee base = room charge excl. tax (today some rows use room + cleaning, a few 12%).
   - Monthly Rental and Long Term Rental: CONFIRM 0% (today 53 monthly rentals and 2 LTR carry 8%).
...
12. Booking.com, Agoda, Expedia, Airbnb, Trip.com, Traveloka, Tiket.com, Website, Monthly Rental, Long Term Rental, Owner.
    eZee's source is mapped to this list (CTrip/Ctrip/Ctrip.com → Trip.com; Traveloka codes stripped;
    Walk In/PMS/Google/Internet/Booking Engine → Website). CONFIRM the list.
